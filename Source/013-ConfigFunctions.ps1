...
function Get-FEDAUTOSettingsCatalog {
    ...
    $items = @(
        ...
        @('Source acquisition','MissingSourceFileAction','Do nothing','When a previously acquired source file is absent from a complete source inventory, Do nothing leaves it unchanged; Rename keeps it in place with a suffix; Relocate moves it to the deleted folder.'),
        @('Source acquisition','MissingSourceFileSuffix','_MISSING','Text inserted before the file extension when MissingSourceFileAction is Rename, for example Model_MISSING.ifc.'),
        @('Source acquisition','ExcludeMissingSourceFiles','Yes','When Yes, files marked with MissingSourceFileSuffix are excluded from IFC data extraction and federation.'),
        ...
        @('IFC Data Extraction','IfcDataExtractionStallWarningMinutes','15','Warn when a worker has not sent a heartbeat for this many minutes. A warning does not stop a busy worker.'),
        @('IFC Data Extraction','IfcDataExtractionStallTimeoutMinutes','30','Stop a worker only after this heartbeat timeout and sustained negligible CPU and disk I/O activity.'),
        @('IFC Data Extraction','IfcDataExtractionFailurePolicy','Continue','Continue records failed IFCs and proceeds; FailPipeline stops the pipeline after extraction failures.'),
        @('IFC Data Extraction','IfcOpenShellVersion','0.8.5','Pinned IfcOpenShell version used for IFC extraction and metadata writing.'),
